...
def make_charts():
    ...
    # Chart 2: latest China indicators
    labels = ["开发投资", "新开工面积", "竣工面积", "销售面积", "销售额"]
    values = [-18.0, -23.4, -23.7, -11.6, -13.6]
    fig, ax = plt.subplots(figsize=(8.3, 4.4))
...
def build_report():
    ...
    add_numbered(doc, [
        ("调整尚未结束，但已进入后半程。", "2025年房地产开发投资同比下降17.2%，新建商品房销售面积下降8.7%；2026年1—6月投资、销售面积和新开工又分别下降18.0%、11.6%和23.4%。供给收缩有利于未来再平衡，但销售弱于库存去化意味着价格仍需时间发现底部。"),
        ...
        ("四城排序：上海≈杭州＞深圳＞北京（基准情景的价格弹性），但风险调整后上海最均衡。", "上海兼具全球城市产业、人口规模和2026年新房相对强势；杭州人口与数字经济增长突出但新房供应结构抬高指数；深圳人口增量最大、周期弹性也最大；北京资源稀缺但人口基本持平、二手房调整更深。"),
    ...
    add_paragraph(doc, "2025年全国房地产开发投资8.28万亿元，同比下降17.2%；新建商品房销售面积8.81亿平方米，同比下降8.7%，销售额8.39万亿元，下降12.6%；年末商品房待售面积7.66亿平方米，同比增长1.6%。2026年上半年，投资与销售继续收缩，说明2024年以来的宽松政策改善了局部城市成交，但尚未改变全国总量趋势。")
    add_picture(doc, p2, "图2　2026年1—6月全国房地产主要指标同比变化", "国家统计局《2026年1—6月份全国房地产市场基本情况》。")
    add_callout(doc, "关键辨析", "新开工下降23.4%不是纯粹利空。短期它反映开发商资金与信心不足；中期则意味着新增供给显著减少，是库存消化和价格稳定的必要条件。真正的转折信号应是“销售降幅收窄 + 待售库存持续下降 + 二手挂牌周期缩短”，而不是单看新开工。")
    ...
    add_table(doc, ["经济体/周期", "上行机制", "调整路径", "政策结果", "对中国启示"], [
        ["日本（1985—约2005）", "金融自由化、低利率、土地抵押信贷、人口结构见顶", "1991后土地与住宅长期下跌；东京实际住宅低点约为峰值1/3（特定指数）", "宽松过慢、坏账确认迟，资产负债表衰退拖长", "及时确认损失、重组开发商和释放价格比长期“保价”更重要"],
        ["美国（1997—2012）", "低利率、宽松承销、证券化、供给弹性", "2006见顶，Case-Shiller全国名义指数峰谷约-27%；止赎加剧下跌", "快速银行资本重整、QE、房贷机构支持；2012后复苏", "金融体系损失确认速度决定宏观拖尾；交易出清先于开发复苏"],
        ["西班牙/爱尔兰（1997—2013）", "欧元低利率、跨境资金、建筑过度扩张", "房价和建设活动大幅回撤；银行与主权风险互相强化", "银行重组、坏账平台、财政约束下缓慢修复", "高库存地区即使降息也难迅速反转，供给出清不可省略"],
        ["德国（2010—2024）", "低利率、城市化、供给约束推动十余年上涨", "2022后利率急升，名义价格温和下调、交易冻结，2024—2025趋稳", "租赁市场深、长期固定融资缓冲系统风险", "租赁现金流和低杠杆可显著降低价格周期的金融放大"],
    ...
    add_paragraph(doc, "注：各国指数口径不同，表中峰谷幅度仅用于说明数量级，不做一一可比。日本“低点约峰值1/3”来自日本银行演讲所引大东京实际公寓指数；美国约-27%为Case-Shiller全国名义指数的常见峰谷估算。", style="Caption Source")
    ...
    doc.add_heading("6.4 买入、谈判与持有流程", level=2)
    add_numbered(doc, [
    ...
    doc.add_heading("6.5 应回避的高风险品类", level=2)
    add_bullets(doc, [
